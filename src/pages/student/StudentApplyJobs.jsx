            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <span className="text-xs font-semibold text-violet-700 uppercase tracking-wider">Career Portal</span>
                </motion.div>
                <h2 className="text-4xl font-extrabold text-[#2F2F5B] tracking-tight">Jobs for you</h2>
              </div>
            </div>
            ...
            <div className="flex gap-6 border-b border-violet-100 mb-8 pb-1">
              {["Recommended", "Saved"].map((tab) => (
                <button
                  ...
                  className={`pb-3 font-semibold text-sm sm:text-base relative transition-colors ${selectedTab === tab
                      ? "text-violet-700"
                  ...
                  {filteredJobs.length > 0 ? (
                    filteredJobs.map((job, index) => (
                      <motion.div
                        ...
                        className={`group cursor-pointer bg-white/80 backdrop-blur border p-5 rounded-2xl flex gap-4 items-start shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden ${selectedJob?.id === job.id
                      ...
                      >
                        ...
                        <div className="flex-1">
                          <h3 className="font-bold text-[#2C225A] text-base mb-1 group-hover:text-violet-700 transition-colors line-clamp-1">{job.job_title}</h3>
                          <p className="text-sm font-medium text-[#4b436f] mb-2">{job.company_name}</p>

                          <div className="flex flex-wrap gap-2 text-xs font-medium">
                            <span className="flex items-center gap-1 bg-gray-100 text-gray-600 px-2 py-1 rounded-md">
                              <FaMapMarkerAlt className="text-gray-400" /> {parseLocation(job.location)}
                            </span>
                            <span className="flex items-center gap-1 bg-red-50 text-red-600 px-2 py-1 rounded-md">
                              <FaCalendarAlt className="text-red-400" /> {job.application_deadline}
                            </span>
                          </div>
                        </div>
              ...
              <div className="lg:col-span-7">
                {selectedJob ? (
                  ...
                  >
                    ...
                    <div className="flex flex-wrap gap-6 items-start justify-between mb-8 pb-6 border-b border-gray-100 relative z-10">
                      <div className="flex gap-5 items-center">
                        ...
                        <div>
                          <h3 className="text-2xl font-extrabold text-[#2F2F5B] mb-1">{selectedJob.job_title}</h3>
                          <p className="text-lg font-medium text-violet-600">{selectedJob.company_name}</p>
                          <p className="text-xs text-gray-400 mt-1 uppercase tracking-wider font-semibold">Job ID: {selectedJob.id}</p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-8 relative z-10">
                      <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100 flex items-start gap-3">
                        <div className="mt-0.5 text-violet-500"><FaMapMarkerAlt /></div>
                        <div>
                          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide">Location</h4>
                          <p className="text-sm font-semibold text-[#2C225A] mt-0.5">{parseLocation(selectedJob.location)}</p>
                        </div>
                      </div>
                      <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100 flex items-start gap-3">
                        <div className="mt-0.5 text-emerald-500"><FaWallet /></div>
                        <div>
                          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide">Salary Package</h4>
                          <p className="text-sm font-semibold text-[#2C225A] mt-0.5">{selectedJob.salary_package}</p>
                        </div>
                      </div>
                      <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100 flex items-start gap-3">
                        <div className="mt-0.5 text-red-400"><FaCalendarAlt /></div>
                        <div>
                          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide">Apply Before</h4>
                          <p className="text-sm font-semibold text-[#2C225A] mt-0.5">{selectedJob.application_deadline}</p>
                        </div>
                      </div>
                      <div className="bg-gray-50/50 p-4 rounded-2xl border border-gray-100 flex items-start gap-3">
                        <div className="mt-0.5 text-blue-500"><FaTasks /></div>
                        <div>
                          <h4 className="text-xs font-bold text-gray-400 uppercase tracking-wide">Recruitment Process</h4>
                          <p className="text-sm font-semibold text-[#2C225A] mt-0.5">{selectedJob.recruitment_process}</p>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="h-full bg-white/40 border border-dashed border-violet-200 rounded-3xl flex flex-col items-center justify-center p-10 text-center">
                    ...
                    <h3 className="text-xl font-bold text-[#2C225A] mb-2">Select a Job Listing</h3>
                    <p className="text-[#4b436f]">Click on any job from the list to view detailed information and apply.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          >
            ...
            >
              <div className="bg-gradient-to-r from-violet-600 to-indigo-600 p-5 pb-8 relative">
                <h3 className="text-xl font-bold text-white tracking-wide">Set Job Preferences</h3>
                <p className="text-violet-100 text-sm mt-1">Filter your job feed based on your interests.</p>
              </div>
