...
const TPOCodingAssessmentForm = () => {
  ...
  const handleDetailsChange = (e) => {
    setAssessmentDetails({ ...assessmentDetails, [e.target.name]: e.target.value });
  };
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <input required type="datetime-local" name="startTime" onChange={handleDetailsChange} value={assessmentDetails.startTime} className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-violet-500 bg-white/50" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-2">End Time</label>
                  <input required type="datetime-local" name="endTime" onChange={handleDetailsChange} value={assessmentDetails.endTime} className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-violet-500 bg-white/50" />
                </div>
              </div>
            </div>
              ...
              {questions.map((q, qIndex) => (
                <div key={qIndex} className="bg-white rounded-2xl p-6 shadow-sm border border-violet-100 relative">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-sm font-bold text-gray-700 mb-2">Description / Problem Statement (Markdown/HTML supported)</label>
                      <textarea required rows="4" value={q.description} onChange={(e) => handleQuestionChange(qIndex, 'description', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-violet-500 resize-none font-mono text-sm"></textarea>
                    </div>

                    <div>
                      <label className="block text-sm font-bold text-gray-700 mb-2">Constraints</label>
                      <textarea rows="2" placeholder="e.g. 2 <= nums.length <= 10^4" value={q.constraints} onChange={(e) => handleQuestionChange(qIndex, 'constraints', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:ring-2 focus:ring-violet-500 resize-none font-mono text-sm"></textarea>
                    </div>

                    {/* Test Cases Nested Array */}
                    <div className="mt-6 pt-6 border-t border-gray-100">
                      <div className="flex justify-between items-center mb-4">
                        <label className="block text-sm font-bold text-indigo-700">Execution Test Cases</label>
                        <button type="button" onClick={() => handleAddTestCase(qIndex)} className="text-xs font-bold bg-indigo-50 text-indigo-600 px-3 py-1 rounded-md hover:bg-indigo-100 flex items-center gap-1">
                        ...
                        {q.testCases.map((tc, tcIndex) => (
                          <div key={tcIndex} className="flex gap-4 items-start bg-gray-50 p-4 rounded-xl border border-gray-200">
                            ...
                            <div className="w-24">
                              ...
                              <input required type="number" min="0" value={tc.marks} onChange={(e) => handleTestCaseChange(qIndex, tcIndex, 'marks', parseInt(e.target.value))} className="w-full p-2 border border-gray-300 rounded-lg text-sm" />
                            </div>
                            <div className="w-24 flex flex-col items-center">
                              <label className="block text-xs font-bold text-gray-600 mb-2">Hidden?</label>
                              <input type="checkbox" checked={tc.isHidden} onChange={(e) => handleTestCaseChange(qIndex, tcIndex, 'isHidden', e.target.checked)} className="w-5 h-5 text-indigo-600 rounded bg-gray-100 border-gray-300 focus:ring-indigo-500" />
                            </div>
                            <button type="button" onClick={() => handleRemoveTestCase(qIndex, tcIndex)} className="text-gray-400 hover:text-red-500 mt-6">
                              <FaTrash size={14} />
                            </button>
                          </div>
                        ))}
