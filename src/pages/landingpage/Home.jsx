...
const Home = () => {
  useEffect(() => {
    window.scrollTo(0, 0); // ensure scroll is reset
  }, []);
  ...
  return (
    <div className="text-[#4A4A7B] bg-soft-glow scroll-smooth overflow-x-hidden overflow-y-hidden">
      ...
      <section
        id="home"
        className="min-h-[calc(100vh-4rem)] pt-28 px-6 md:px-20 flex flex-col md:flex-row items-center justify-between gap-10">
        ...
        >
          <motion.h1
            className="text-4xl md:text-6xl font-bold leading-tight mb-6 text-[#2F2F5B]"
          ...
          <p className="text-[#5C5C80] mb-8">
            CareerNexus helps you manage applications, resumes, deadlines, and analytics — all in one place.
          </p>
      ...
      <section
        id="features"
        className="py-20 px-6 sm:px-10 md:px-20 bg-gradient-to-br from-[#F8E5EB] to-[#E4EBFE]"
      >
        <motion.h2
          ...
          {features.map((feature, index) => (
            ...
            >
              <img
                src={feature.img}
                alt={feature.title}
                className="w-full h-48 object-cover rounded-lg mb-5 shadow-md group-hover:shadow-violet-400/30 transition"
              />
      ...
      <section
        id="about"
        className="py-20 px-6 sm:px-10 md:px-24 bg-gradient-to-br from-[#F8E5EB] to-[#E4EBFE]"
      >
        <motion.div
